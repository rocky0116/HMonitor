...
class DOPDropDownMenu: UIView, UITableViewDataSource, UITableViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate func calculateTitleSizeWithString(string: String) -> CGSize {
        let dic = [kCTFontAttributeName:UIFont.systemFont(ofSize: CGFloat(fontSize))]
        let size: CGSize = string.boundingRect(with: CGSize(width: 280, height: 0), options:NSStringDrawingOptions.usesLineFragmentOrigin, attributes: dic as [NSAttributedStringKey : Any], context: nil).size
