...
class WBOAuthViewController: UIViewController {
    ...
    @objc func autoFill() {
        ...
        // 准备 js
        let js = "document.getElementById('userId').value = '15835264250';" +
        "document.getElementById('passwd').value = 'zhong3344';"
...
extension WBOAuthViewController: UIWebViewDelegate {
    ...
    func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
        ...
        HMNetworkManager.shared.loadAccessToken(code: code) { (isSuccess) in
            
            if !isSuccess {
                SwiftProgressHUD.showInfo("网络请求失败")
            } else {
